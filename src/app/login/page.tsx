    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "var(--sidebar)" }}>
      <div className="card w-full max-w-md p-8">
        ...
        <p className="mt-6 text-xs text-muted">
          {googleEnabled
            ? "Access is invite-only. Sign in with the Google account an administrator invited; read-only Sheets access is requested for sync."
            : "Development sign-in (local only). Configure Google OAuth for invite-only Google sign-in and Sheets sync."}
        </p>
      </div>
    </div>
  );
